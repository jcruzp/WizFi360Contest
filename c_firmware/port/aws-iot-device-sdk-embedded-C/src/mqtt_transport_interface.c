...
int8_t mqtt_transport_connect(uint8_t ssl_flag, uint8_t *recv_buf, uint32_t recv_buf_len, uint8_t *domain, uint32_t port, tlsContext_t *tls_context,MQTTEventCallback_t userCallBack)
{
    ...
    if (ssl_flag == 0)
    {
        ...
        ret = iotSocketGetHostByName (domain, af, ip_addr, &ip_len);
        if (ret < 0)
            return (-1);
        ...
        if (iotSocketConnect (sock, ip_addr, 4, port) < 0) {
            iotSocketClose (sock);
            return (-1);
        }
    ...
    else
    {
        ...
        ret = ssl_socket_connect(tls_context, domain, g_mqtt_config.mqtt_ip, port);
        if (ret != 0)
        {
            mqtt_transport_close(sock, &g_mqtt_config);
...
int32_t mqtt_write(NetworkContext_t *pNetworkContext, const void *pBuffer, size_t bytesToSend)
{
    ...
    size = iotSocketSend(pNetworkContext->socketDescriptor, (uint8_t *)pBuffer, bytesToSend);
...
{
    int32_t ret;
    uint32_t n = MQTT_TIMEOUT;

    ret = iotSocketSetOpt ((uint8_t)pNetworkContext->socketDescriptor, IOT_SOCKET_SO_RCVTIMEO, &n, sizeof(n));
    if (ret < 0)
        return (-1);

    ret = iotSocketRecv ((uint8_t)pNetworkContext->socketDescriptor, pBuffer, bytesToRecv);
    if (ret < 0)
        return (-1);
